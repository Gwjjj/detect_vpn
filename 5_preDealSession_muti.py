...
def start_deal():
    ...
    try:
        for i in range(flow_class):
            ...
            vpn_pickle_save_route = os.path.join(flow_pickle_save_dir, filename)
...
def deal_pcap_files(dir_route, dir):
    for in_root, _, pcap_files in os.walk(dir_route):
        if dir in TYPES:
            try:        
                _, category_label, _ = TYPES[dir] # 分类
                ...
                for item in pcap_file_size_list:
                    ...
                    if res:
                        save_category(session_list, category_label)
...
def save_category(session_list, category_label):
...
def deal_pcap_frame(pcap_file_route, session_list):
    ...
    try:
        for packet in PcapReader(pcap_file_route):
            ...
            data = np.frombuffer(raw_content, dtype=np.uint8, count=frame_max_length) # 16进制转换10进制
