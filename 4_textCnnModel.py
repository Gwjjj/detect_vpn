...
attention_dim = int(mu.getConfig('parameter', 'attention_dim'))
...
def attention(x_i, x, index):
        ...
        for output in x:
            ...
            atten_hidden = tf.tanh(tf.add(tf.matmul(x_i, attention_W), tf.matmul(output, attention_U)))
            e_i_j = tf.matmul(atten_hidden, attention_V)
            e_i.append(e_i_j)
        e_i = tf.concat(e_i, axis=1)
        # e_i = tf.exp(e_i)
        alpha_i = tf.nn.softmax(e_i)
        alpha_i = tf.split(alpha_i, sequence_length, 1)
        ...
        for j, (alpha_i_j, output) in enumerate(zip(alpha_i, x)):
            if j == index:
                ...
                c_i.append(c_i_j)
        c_i = tf.reshape(tf.concat(c_i, axis=1), [-1, sequence_length-1, embedding_size])
...
# frame_dou_length = frame_max_length

if use_attention:
    embedding_size = frame_max_length
    attention_hidden_dim = attention_dim
    # Wa = [attention_W  attention_U]
    attention_W = tf.Variable(
        tf.random_uniform([embedding_size, attention_hidden_dim], 0.0, 1.0),
        name="attention_W")
    attention_U = tf.Variable(
        tf.random_uniform([embedding_size, attention_hidden_dim], 0.0, 1.0),
        name="attention_U")
    attention_V = tf.Variable(tf.random_uniform([attention_hidden_dim, 1], 0.0, 1.0),
                                    name="attention_V")
    # attention layer before convolution
    output_att = list()
    with tf.name_scope("attention"):
        input_att = tf.split(x, sequence_length, axis=1)
        for index, x_i in enumerate(input_att):
            x_i = tf.reshape(x_i, [-1, embedding_size])
            c_i = attention(x_i, input_att, index)
            inp = tf.concat([x_i, c_i], axis=1)
            output_att.append(inp)

        input_conv = tf.reshape(tf.concat(output_att, axis=1),
                                [-1, sequence_length, embedding_size*2],
                                name="input_convolution")
    input_conv_expanded = tf.expand_dims(input_conv, -1)
    # frame_dou_length = frame_max_length * 2
else:
    input_conv_expanded = tf.expand_dims(x, -1)
    
dim_input_conv = input_conv_expanded.shape[-2]
...
# model_saver = "vpn_novpn" + str(frames_min_threshold) +'_' + str(frames_max_threshold) 
...
saver = tf.train.Saver(max_to_keep=1)
model_save_path = "vpn_novpn" + str(frames_min_threshold) +'_' + str(frames_max_threshold) +'_new'+ "/model"
