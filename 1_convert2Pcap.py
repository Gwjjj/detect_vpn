...
if __name__ == '__main__':
    # Example:
    dirname = mu.getConfig('URI', '1_ConvertDirName')
    pattern = os.path.join(dirname, '*\\', '*.pcapng')
    files = glob.glob(pattern)
    for filename in files:
        convert_filename = filename[:-2]
        psshell = 'editcap.exe -F pcap ' + filename + ' ' + convert_filename
